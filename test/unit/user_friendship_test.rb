...
class UserFriendshipTest < ActiveSupport::TestCase
	...
	test "that creating a friendship works without raising an exception" do
		assert_nothing_raised do
			UserFriendship.create user: users(:logan), friend: users(:mike)
		end
	end
	...
	context "a new instance" do
		setup do
			@user_friendship = UserFriendship.new user: users(:logan), friend: users(:mike)
		end
	...
	context "#send_request_email" do
		setup do
			@user_friendship = UserFriendship.create user: users(:logan), friend: users(:mike)
		end
	...
	context ".request" do
		should "create two user friendships" do
			assert_difference "UserFriendship.count", 2 do
				UserFriendship.request(users(:logan), users(:mike))
			end
		end
