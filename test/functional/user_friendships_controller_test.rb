...
class UserFriendshipsControllerTest < ActionController::TestCase
  context "#index" do
    ...
    context "when logged in" do
      setup do
        @friendship1 = create(:pending_user_friendship, user: users(:logan), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
        @friendship2 = create(:accepted_user_friendship, user: users(:logan), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
        sign_in users(:logan)
        get :index
      end
  ...
  context "#new" do
  	...
  	context "when logged in" do
  		...
  		should "assign a new user friendship to the currently logged in user" do
  			get :new, friend_id: users(:jim)
  			assert_equal users(:logan), assigns(:user_friendship).user
  		end
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @user_friendship = create(:pending_user_friendship, user: users(:logan))
        sign_in users(:logan)
